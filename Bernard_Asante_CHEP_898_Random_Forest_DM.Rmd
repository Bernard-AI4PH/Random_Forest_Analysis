...
Here, i want to observe how the values of the predictor variable is arranged. From the dataset, which of  the responses("yes" and "No") is set as a preference. That is when used in a table or graph(barplot), which response will  come first. 
...
The main dataset has to be split into 70% for train data and 30% for test data. Strata = "diabetes" ensures equal split of the predictor in test and training dataset 
...
The best model is preprocessor1_model41 because it has the best hyperparameters values as seen on the plot. mtry 13,trees = 100 and  min_n of 35.
...
```
From the ROC_CURVE, the area under the curve for the default is bigger compared to the tuned models. This means the default model had better overall prediction ability compared to the tuned model. 
